<?php 
  ...
  $sql="SELECT PID,PNAME,PRATE,PIMG FROM product WHERE PID='{$_GET["id"]}'";
    $res=$con->query($sql);
    if($res->num_rows>0){
        $r=$res->fetch_assoc();
    }
...
<body>    
    ...
    <div class="container-fluid mt-4" style="font-family: Cambria, Cochin, Georgia, Times, 'Times New Roman', serif;">
    <h4>Pick Your Products !</h4> 
            <div class="row">
                ...
                <div class="col-md-1">
                    ...
                    <?php 
                        $sql="SELECT * FROM pro_img  WHERE PID={$_GET["id"]}";
                        $res=$con->query($sql);
                        if($res->num_rows>0){
                ...
                <div class="col-md-4">
                        <img src="<?php echo $r["PIMG"]; ?>" alt="" width="100%" height="350px"> 
                </div>
                <div class="col-md-7">
                    <h3><?php echo $r["PNAME"]; ?>
                         <?php
                    ...
                    <div class="col-md-6">
                        <ul>
                            <?php 
                                $sql="SELECT * FROM pro_des WHERE PID={$_GET["id"]}";
                                $res=$con->query($sql);
                                if($res->num_rows>0){
            ...
            <div class="row">
                <div class="col-md-5">
                    ...
                    <h5>Features</h5>
                    <?php 
                        $sql="SELECT * FROM pro_fus  WHERE PID={$_GET["id"]}";
                        $res=$con->query($sql);
                        if($res->num_rows>0){
                ...
                //      while($rimg=$res->fetch_assoc()){
                        ...
                        INNER JOIN register r ON c.RID=r.RID WHERE c.PID={$_GET["id"]}";
                        $res=$con->query($sql);
                        if($res->num_rows>0){
            ...
            // }
                ?>
                    ...
                    <?php      
                        ...
                        $sql="INSERT INTO cmt(PID,RID,CMNT) VALUES('{$_GET["id"]}',
                        '{$_SESSION["rid"]}',
                        '{$cmnt}')";
                        if($con->query($sql)){
                    ...
                    <form action="<?php echo $_SERVER ["REQUEST_URI"] ?>" method="post" 
                        ...
                        <div class="form-group">
                            <textarea name="cmnt" placeholder="Send your comments" id="" cols="40" rows="2"></textarea>
                        </div>
                        <button type="submit" class="btn btn-info" name="send">Send Reviews</button>
                    </form>                        
                    </div>
                </div>              
